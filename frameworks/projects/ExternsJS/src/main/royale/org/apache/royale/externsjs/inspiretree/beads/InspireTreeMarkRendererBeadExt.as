...
package org.apache.royale.externsjs.inspiretree.beads
{
	...
	public class InspireTreeMarkRendererBeadExt extends InspireTreeMarkRendererBead
	{
		...
		override public function set markDOMFunction(value:Function):void
		{
			...
			if(treeModel)
				InspireTreeModelExt(treeModel).markDOMFunction = value;
		}
		...
		public function set markIsDisabled(value:Boolean):void
		{
			...
			if(treeModel)
				InspireTreeModelExt(treeModel).markIsDisabled = value;
		}
		...
		public function set markToState(value:String):void
		{
			_markToState = value;
			if(treeModel)
				InspireTreeModelExt(treeModel).markToState = value;
		}
		...
		override protected function init(event:Event):void
		{
			...
			if(treeModel)
			{
				...
				if(!InspireTreeModelExt(treeModel).markDOMFunction && _markDOMFunction)
					InspireTreeModelExt(treeModel).markDOMFunction = _markDOMFunction;

				if( _markToState )
					InspireTreeModelExt(treeModel).markToState = _markToState;

				InspireTreeModelExt(treeModel).markIsDisabled = _markIsDisabled;
		...
		override protected function updateHost(event:Event = null):void
		{
			...
			for (var idxnode:int=0; idxnode < lenar; idxnode++)
			{
				...
				if(treenode.hasChildren())
				{
					...
					{
						itreal = (treeModel.dataProviderTree[idxnode] as NormalizedDataItem).children[idxChild];
						marked = markDOMFunction(treenode.children[idxChild],(itreal as NormalizedDataItem).data);
					...
					if(arAct.length == 0) //Mark root
					{
						...
						if(treeModel.checkboxMode && InspireTreeModelExt(treeModel).markIsDisabled && arAct.length<lench)
						{
							var checkbox:HTMLElement = titlerow.children[1] as HTMLElement;
							checkbox.setAttribute('data-uid', row.getAttribute('data-uid'));
...
*/
		...
		private function checkRootMark(event:Event):void
		{
			...
			var newstate:String;
			if( (vNode.props.indeterminate as Boolean) == true || (vNode.props.checked as Boolean) == true )
				newstate = "unchecked";
			else
				newstate = "checked";
			...
			// no funciona porque ahora no existe event.path: var idNode:String = event.path[2].attributes[1].value as String;
			
			// Hemos añadido el atributo data-uid al checkbox, al crearlo:
			var idNode:String = (event.currentTarget as Element).getAttribute('data-uid');
